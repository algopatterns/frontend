...
This document describes how the frontend should interact with the Algopatterns API, covering authentication, sessions, and the distinction between REST and WebSocket usage.
...
```
wss://algopatterns.cc/ws?session_id={uuid}&token={jwt}&display_name={name}
```
...
```
URL format: https://algopatterns.cc/join?invite={invite_token}

...
export function AIInput({ onSendAIRequest, disabled = false }: AIInputProps) {
  ...
  const { aiDrawerHeight } = useUIStore();
    ...
    <div
      ...
      {isExpanded && conversationHistory.length > 0 && (
        <div className="border border-border rounded-xl mt-1 flex flex-col min-h-0 overflow-hidden" style={{ height: drawerHeight }}>
          ...
          <div ref={contentRef} className="overflow-y-auto p-3 space-y-2 flex-1 min-h-0">
      ...
      <div className="h-16 flex items-center shrink-0">
        <div className="bg-muted/50 border border-muted rounded-lg px-3 py-2 flex items-center gap-2 w-full">
          <input
            ...
            placeholder="Ask algopatterns agent for help or code snippets..."
